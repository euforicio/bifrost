...
Classification runs only when a routing rule actually references `complexity_tier`, so requests that never touch a complexity rule pay no embedding cost. Once semantic classification is configured, a request it cannot confidently match, such as a near miss or timeout, leaves `complexity_tier` unpublished. You can optionally configure an **LLM fallback classifier** or a **Jev classifier** to step in after semantic classification has run and returned no tier. See [LLM fallback classifier](#llm-fallback-classifier) and [Jev classifier](#jev-classifier). Jev can also be the primary classifier when no semantic block is configured. Without any classifier configured, Bifrost keeps the request on its existing routing path instead of guessing. When [session-aware routing](#session-aware-routing) is enabled and the request carries a recognized session identity, a turn that still produces no tier of its own reuses the tier already retained for that session, so `complexity_tier` goes unpublished only when neither classification nor session state supplies one.
...
[TypeSafe Jev](/providers/supported-providers/typesafe) is a native System One evaluate model, not a chat fake. The Complexity Router uses the configured `jev.provider` + `jev.model` through Bifrost's provider queue (keys, retries, `network_config`). There is no hard-coded HTTP call that skips Providers config. The TypeSafe key must be `env.TYPESAFE_API_KEY` (or a Bifrost key-store entry that resolves to that env) — never a literal secret in config.json, the UI, CI, or the repo.

Two clean paths:

- **Primary** — configure a `jev` block and omit `semantic`. Jev classifies every request that needs a tier.
- **Semantic fallback** — set `semantic.fallback` to `jev`. Jev runs only after semantic classification produces no tier, the same way the LLM fallback does.

One System One call asks a contrastive Choice `SIMPLE|MEDIUM|COMPLEX` plus `OTHER`/`UNKNOWN` (so a weak match does not force a tier), an optional parallel Noul (`needs_frontier`), and an optional companion Score (`complexity_score`, 1–3). Bifrost then **composes in code**:

- If Choice is `OTHER` or `UNKNOWN`, `complexity_tier` is left unpublished.
- If `Choice.confidence` is below `min_confidence` (default `0.6`), `complexity_tier` is left unpublished — same degrade as a semantic miss. Do not force a tier.
- Otherwise the Choice is mapped to the uppercase CEL tier. Score never names a discrete tier on its own.
- Optional: a MEDIUM Choice with middling confidence plus high `needs_frontier.noul` or a high Score (after the Score confidence gate) is escalated to COMPLEX with a logged reason.

State is always the **latest user message**. History is filtered in code; Jev is not asked to do math, date arithmetic, or count turns. The response `model` (versioned ID), Choice `confidence`, and Choice `probabilities` are logged. No invented similarity scores. The classifier runs **at most once per request**.

Config examples default to the `jev-latest` alias. Tests and httptest fixtures pin `jev-1.13.0`.

<Warning>
On the model-routing path, Jev timeout, 401/missing key, `429`, `529`, and other errors **fail open**: no `complexity_tier` is published and the request stays on the existing default path (same as a semantic miss). Safety and tool gates (future) may fail closed. Session routing may still reuse a retained tier.
</Warning>

Low-cost live smoke (Jev classifier + Fireworks Flash for SIMPLE/MEDIUM + one `grok-3-mini` COMPLEX call) is documented on [TypeSafe](/providers/supported-providers/typesafe#low-cost-complexity-router-smoke). Keys stay `env.TYPESAFE_API_KEY`, `env.FIREWORKS_API_KEY`, and `env.XAI_API_KEY`. Do not soak, and do not route COMPLEX to `grok-4.6`.
...
- **Session-aware routing**: retain the highest tier reached by each identified session for 24 hours of inactivity. The toggle is off by default and requires a semantic or Jev classifier.
- **Jev classifier**: provider, model (`jev-latest` or `jev-1.13.0`), timeout, minimum Choice confidence, and budget attribution. Primary when embeddings are not configured; fallback when **When no phrase matches confidently** is **Jev classifier**.
- **Edit embedding configuration**: opens the embedding sheet (provider, model, similarity floor, history window, timeout, budgets, and phrase storage: **Embedded** keeps phrase vectors in Bifrost's own memory; **Vector Store** keeps them in the configured vector store so they survive restarts, falling back to Embedded when none is available). Setting **When no phrase matches confidently** to **LLM classifier** or **Jev classifier** selects the matching fallback. Jev settings live on the main page; LLM model settings appear in the sheet.
...
# Enable the Jev classifier as semantic fallback (or omit semantic for Jev-primary)
curl -X PUT http://localhost:8080/api/routing/complexity-analyzer-config \
  -H "Content-Type: application/json" \
  -d '{
    "semantic": {
...
| `semantic.fallback` | string | `none` | What answers when semantic classification produces no tier: `none` records the request as skipped; `llm` asks the chat model in `llm`; `jev` asks TypeSafe System One using `jev`. Requires the matching block |
...
| `jev.provider` | string | Required when Jev is primary or `fallback` is `jev` | Provider used to run System One (typically `typesafe`); the key value must be `env.TYPESAFE_API_KEY` or a key-store pointer to that env |
| `jev.model` | string | Required when Jev is used | Jev model (`jev-latest` or a versioned id such as `jev-1.13.0`) |
| `jev.timeout` | duration | `1.5s` | Ceiling on the System One call; exceeding it skips tier routing for that request |
| `jev.min_confidence` | number | `0.6` | Choice.confidence floor. Below it no `complexity_tier` is published |
| `jev.message_history_count` | integer | `1` | Must be `1`. Jev always evaluates the latest user message only |
| `jev.count_toward_budgets` | boolean | `false` | Count System One classification cost toward virtual-key budgets (record-only, never enforced) |
| `session.enabled` | boolean | `false` | Retain the highest observed tier across normally sequential turns for 24 hours of inactivity. Requires `semantic` or `jev`; overlapping requests for the same session are best-effort |
...
| `complexity_mechanism` | `semantic`, `llm`, `jev`, `session`, `skipped` | How the effective tier was produced. `semantic` means an embedding match supplied it; `llm` means the fallback chat model named it; `jev` means TypeSafe System One named it; `session` means retained session state supplied it because the current turn was a continuation, proposed a lower tier, produced no tier, or followed the Complex ceiling; `skipped` means a rule demanded a tier but neither a classifier nor existing session state produced one |
| `complexity_score` | 0.0 – 1.0 | For `semantic`, the similarity of the nearest reference phrase. For `jev`, the documented Choice.confidence. Choice `probabilities` are written on the routing-engine log line, not invented as a similarity. Absent for `llm`, `session`, and `skipped` |
...
<Note>
The raw `complexity_score` is displayed but not filterable; tier and mechanism are the supported filter dimensions. The mechanism filter offers `semantic`, `llm`, `jev`, `session`, and `skipped`. Legacy `REASONING` tiers remain available in the logs filter.
</Note>
