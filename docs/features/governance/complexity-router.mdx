...
One System One call asks a contrastive Choice `SIMPLE|MEDIUM|COMPLEX` plus an optional parallel Noul (`needs_frontier`). Bifrost then **composes in code**:
...
- Otherwise the Choice is mapped to the uppercase CEL tier.
- Optional: a MEDIUM Choice with high `needs_frontier.noul` and middling confidence is escalated to COMPLEX with a logged reason.

State is always the **latest user message**. History is filtered in code; Jev is not asked to do math, date arithmetic, or count turns. The response `model` (versioned ID) and Choice `confidence` are logged. No invented similarity scores.

<Warning>
A missing or unauthorized TypeSafe key **fails closed** when Jev is selected: the request is rejected rather than routed without a tier. Timeout, `429`, and `529` degrade like the LLM fallback (no tier published; session routing may still reuse a retained tier).
</Warning>
...
| `complexity_score` | 0.0 – 1.0 | For `semantic`, the similarity of the nearest reference phrase. For `jev`, the documented Choice.confidence. Absent for `llm`, `session`, and `skipped` |
