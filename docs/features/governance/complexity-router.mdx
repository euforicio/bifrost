...
[TypeSafe Jev](/providers/supported-providers/typesafe) is a native System One evaluate model, not a chat fake. The Complexity Router uses the configured `jev.provider` + `jev.model` through Bifrost's provider queue (keys, retries, `network_config`). There is no hard-coded HTTP call that skips Providers config.
...
| `jev.provider` | string | Required when Jev is primary or `fallback` is `jev` | Provider used to run System One (typically `typesafe`); must have an enabled key |
