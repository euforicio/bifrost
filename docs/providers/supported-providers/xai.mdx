...
<Note>
For connected Grok subscription accounts, `GET /api/providers/xai/credentials/{credential_id}/usage` returns the current included-usage period, reset time, plan, prepaid balance, and on-demand spending state reported by xAI's Grok client service. Usage refreshes are read-only and never disconnect an otherwise usable inference credential when the billing service is unavailable. xAI does not expose a supported write API for subscription spending settings, so the account card opens Grok's usage settings to enable pay-as-you-go or change its cap.
</Note>
