...
<Note>
xAI does not expose a programmatic account quota endpoint. `GET /api/providers/xai/credentials/{credential_id}/usage` therefore returns `availability: unsupported` with no network probe and does not fabricate quota or credit values. This does not affect xAI inference availability.
</Note>
